...
function abandonedCartTemplate(cart: any, step: number) {
  ...
  const content = `
    ...
    <p class="text">${
      step === 1 
        ...
        : step === 2 
        ? 'Votre sélection de produits suédois est toujours disponible. Ne laissez pas partir ces saveurs authentiques !'
    ...
    <div style="text-align:center">
      <a href="${FRONT_URL}/boutique.html" class="btn">Terminer ma commande →</a>
    </div>
  ...
  return { html: baseTemplate(content, subject), subject };
}

function campaignTemplate(campaign: any) {
  const content = campaign.content || `
    <h1 class="title">${campaign.name}</h1>
    <p class="text">Découvrez nos nouveautés et offres exclusives.</p>
    <div style="text-align:center">
      <a href="${FRONT_URL}/boutique.html" class="btn">Voir la boutique →</a>
    </div>`;
  return baseTemplate(content, campaign.subject || campaign.name);
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (type === 'abandoned_cart') {
      ...
      const { html, subject } = abandonedCartTemplate(cart, step || 1);
      await sendEmail({ to: cart.customer_email, subject, html });
    ...
    if (type === 'campaign') {
      ...
      let sent = 0;
      const html = campaignTemplate(campaign);
      for (const email of emails) {
        try {
          await sendEmail({ to: email, subject: campaign.subject || campaign.name, html });
          sent++;
